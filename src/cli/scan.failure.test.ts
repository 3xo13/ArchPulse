...
beforeEach(() => {
  ...
  vi.mocked(spawnSync).mockImplementation((_file, args) => result(
...
it("uses relative scope and the bundled version in an external repo without dependencies", async () => {
  ...
  expect(call[2]).toMatchObject({ shell: false, cwd: temp });
});
it.each([
  ...
  vi.mocked(spawnSync).mockReturnValueOnce(result(stdout, status));
...
it("rejects process errors even with valid stdout", async () => {
  vi.mocked(spawnSync).mockReturnValueOnce({ ...result(JSON.stringify(raw())), error: new Error("launch failed") });
...
it("accepts exit 1 only with real error violations and reports unresolved edges", async () => {
  ...
  vi.mocked(spawnSync).mockReturnValueOnce(result(JSON.stringify(data), 1));
  const summary = await runScan({ repoRoot: temp });
